...
#ifdef CONFIG_RISCV_TRUSTED_VM
static void sbi_dbcn_console_write_secure(struct console *con, const char *s,
					  unsigned n)
{
...
#ifdef CONFIG_RISCV_TRUSTED_VM
		if (is_secure_guest()) {
			ret = sbi_teeg_share_memory(__pa(dbcn_buf),
						    DBCN_BOUNCE_BUF_SIZE);
			if (ret)
				return ret;

			device->con->write = sbi_dbcn_console_write_secure;
			return 0;
		}
#endif
