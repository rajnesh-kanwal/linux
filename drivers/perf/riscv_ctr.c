...
static int get_nr_valid_ctr_entries(void)
{
	int i = 0;
 ...
 * costly.
 */
static int save_new_entries(struct riscv_perf_task_context *ctx, unsigned int depth)
{
...
static void ctr_regset_to_branch_entry(struct cpu_hw_events *cpuc,
				       ...
				       struct ctr_regset *regset, unsigned int idx)
{
...
static void process_new_ctr_entries(struct cpu_hw_events *cpuc,
		struct perf_event *event, unsigned int depth)
{
