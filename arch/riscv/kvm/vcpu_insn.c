			      ...
			      unsigned long htinst)
{
	...
	if (kvm_riscv_nacl_available()) {
		nshmem = nacl_shmem();
		data = nacl_shmem_gpr_read_tee(nshmem,
	...
	if ((insn & INSN_MASK_SW) == INSN_MATCH_SW) {
		...
		len = 8;
		if (kvm_riscv_nacl_available()) {
			data64 = nacl_shmem_gpr_read_tee(
				nshmem, REG_INDEX(insn, SH_RD) * 8 +
						KVM_ARCH_GUEST_ZERO);
		} else {
...
int kvm_riscv_vcpu_mmio_return(struct kvm_vcpu *vcpu, struct kvm_run *run)
{
	...
	case 1:
		...
		if (kvm_riscv_nacl_available()) {
			nacl_shmem_gpr_write_tee(nshmem,
