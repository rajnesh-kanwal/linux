...
static void kvm_riscv_vcpu_update_timedelta(struct kvm_vcpu *vcpu)
{
	...
	/* For TEE VM's HOST doesn't manage HTIMEDELTA. */
	if (is_tee_vcpu(vcpu))
		return;
...
void kvm_riscv_guest_timer_init(struct kvm *kvm)
{
	...
	if (is_tee_vm(kvm)) {
		/* For TEE VMs htimedelta is managed by TSM and it's communicated using
		 * NACL shmem interface when first time VCPU is run. so we read it in
		 * kvm_riscv_tee_vcpu_switchto() where we enter VCPUs.
		 */
		gt->time_delta = 0;
	} else {
