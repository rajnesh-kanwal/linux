...
void kvm_arch_vcpu_postcreate(struct kvm_vcpu *vcpu)
{
	...
	if (unlikely(is_tee_vcpu(vcpu))) {
		rc = kvm_riscv_tee_vcpu_init(vcpu);
		if (rc)
			pr_err("%s: tee vcpu init failed %d\n", __func__, rc);
	}
...
void kvm_arch_vcpu_destroy(struct kvm_vcpu *vcpu)
{
	...
	if (unlikely(is_tee_vcpu(vcpu)))
		kvm_riscv_tee_vcpu_destroy(vcpu);
					      ...
					      struct kvm_cpu_trap *trap)
{
	...
	if (kvm_riscv_nacl_available()) {
		...
		if (is_tee_vcpu(vcpu)) {
			kvm_riscv_tee_vcpu_switchto(vcpu, trap);
		} else {
			kvm_riscv_nacl_vcpu_switchto(vcpu, trap);
		}

		/* Only HTVAL/HTINST is udpated in shared memory by TSM */
