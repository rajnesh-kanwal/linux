				    ...
				    struct kvm_cpu_trap *utrap, bool *exit)
{
	...
	case SBI_EXT_TEEG_UNSHARE_MEMORY:
		...
		for (i = 0; i < num_pages; i++) {
			ret = kvm_riscv_tee_unshare_page(
				vcpu, cp->a0 + i * PAGE_SIZE);
			if (ret)
				return ret;
		}
...
unsigned long kvm_sbi_ext_teeg_probe(struct kvm_vcpu *vcpu)
{
	/* KVM TEEG SBI handler is only meant for handling calls from TSM */
	return 0;
}
